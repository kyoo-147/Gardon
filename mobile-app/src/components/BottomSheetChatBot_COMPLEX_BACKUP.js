...
const BottomSheetChatBot = ({ isVisible, onClose }) => {
  ...
  // State to track current height for layout calculations
  const [currentSheetHeight, setCurrentSheetHeight] = useState(minHeight);

  // Reasonable bottom sheet configuration
  const maxHeight = screenHeight * 0.9; // 75% maximum
  const minHeight = screenHeight * 0.9; // 50% minimum - reasonable starting point
  ...
  // Keyboard handling - adjust to expanded height when needed
  useEffect(() => {
    const keyboardWillShow = Keyboard.addListener(
      Platform.OS === 'ios' ? 'keyboardWillShow' : 'keyboardDidShow',
      (event) => {
        if (isVisible) {
          // Expand to max height when keyboard appears
          currentHeight.current = maxHeight;
          setCurrentSheetHeight(maxHeight);
    ...
    const keyboardWillHide = Keyboard.addListener(
      ...
      () => {
        if (isVisible) {
          // Return to min height when keyboard hides
          currentHeight.current = minHeight;
          setCurrentSheetHeight(minHeight);
          Animated.spring(translateY, {
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      // FIXED: Extract text properly from response
      let responseText = 'Xin lỗi, tôi không thể xử lý yêu cầu này lúc này.';
      ...
      try {
        ...
        responseText = 'Xin lỗi, có lỗi xảy ra khi xử lý phản hồi.';
      }
      
      // Ensure it's always a string
      if (typeof responseText !== 'string' || !responseText.trim()) {
        responseText = 'Xin lỗi, tôi không thể xử lý yêu cầu này lúc này.';
      }
  ...
  return (
    <View style={{ 
      ...
      >
        ...
        {/* Messages Container - Takes exactly available space */}
        <View style={{ 
          height: currentSheetHeight - 140, // Subtract header(70) + input(70) heights
